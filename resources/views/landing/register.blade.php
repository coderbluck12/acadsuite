...
<div class="register-hero text-center mb-0">
    ...
    <p class="opacity-90">Your portal will be live at <code style="background:rgba(255,255,255,0.15);padding:0.2rem 0.6rem;border-radius:5px;">yourname.{{ config('app.base_domain') }}</code></p>
</div>
    ...
    <div class="card form-card p-4 p-md-5">
        ...
        <form method="POST" action="{{ route('landing.register.store') }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="owner_name" class="form-control" value="{{ old('owner_name') }}" required placeholder="e.g. Dr. Jane Doe">
            </div>
            <div class="mb-3">
                <label class="form-label fw-semibold">Suite / Portal Name *</label>
                <input type="text" name="name" class="form-control" value="{{ old('name') }}" required placeholder="e.g. Dr. Jane Doe Academic Hub">
            </div>
            <div class="mb-3">
            ...
            <div class="mb-4">
                ...
                <div class="input-group">
                    <input type="text" name="subdomain" id="subdomainInput" class="form-control" value="{{ old('subdomain') }}" required placeholder="yourname" pattern="[a-z0-9\-]+" style="text-transform:lowercase;">
...
<script>
    ...
    input.addEventListener('input', function() {
        ...
        timer = setTimeout(() => {
            fetch(`{{ route('landing.check-subdomain') }}?subdomain=${val}`)
                .then(r => r.json())
                .then(data => {
